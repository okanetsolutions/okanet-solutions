...
@section('account-content')
    <div class="account-narrow">
        <a class="text-link" href="{{ route('security') }}" wire:navigate>← Ciberseguridad</a>
        ...
        <form method="post" action="{{ route('register') }}" class="account-form account-register-form">
            ...
            <label class="account-check"><input type="checkbox" name="terms" value="1" required @checked(old('terms'))><span>Acepto los <a href="{{ route('terms') }}" wire:navigate>términos del servicio</a> y he leído la <a href="{{ route('privacy') }}" wire:navigate>política de privacidad</a>.</span></label>
            <button class="button" type="submit">Crear cuenta y verificar correo</button>
        </form>
        <p>¿Ya tienes cuenta? <a class="text-link" href="{{ route('login') }}" wire:navigate>Inicia sesión</a></p>
    </div>
@endsection
